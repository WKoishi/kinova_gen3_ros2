                        ...
                        if line:
                            ...
                            if len(line_struct) == 2:
                                if "m_service" in line_struct[0] and "->create_service(" in line_struct[1]:
                                    template_detect = line_struct[0].replace("m_service", '')
                                    template_detect = template_detect.strip('\t')
                                    template_detect = template_detect.strip(' ')
                                    _, _, template_detect = msg_name_format_check(template_detect)

                                    if template_detect in ros2_msg_srv_list[MSG_OR_SRV]:
                                        file_change_flag = True
                                        template_add = "create_service<kortex_driver::" + MSG_OR_SRV + "::" + template_detect + ">"

                                        new_line = line.replace("create_service", template_add)
                                        f_contents = f_contents.replace(line, new_line)
                                        print(template_add, file_path_struct[-1])
